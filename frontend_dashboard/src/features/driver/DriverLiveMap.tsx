...
export function DriverLiveMap() {
  ...
  const { data: assignment, isLoading } = useQuery({
    queryKey: ['driverAssignmentMap'],
    queryFn: driverApi.getMyAssignment,
  });

  // Fetch stops from assignment since driver assignment includes stops!
  useEffect(() => {
    if (assignment?.stops) {
        const sortedStops = [...assignment.stops].sort((a: any, b: any) => a.sequence_number - b.sequence_number);
  ...
  useEffect(() => {
    if (!assignment?.route?.id) return;
    wsService.connect(assignment.route.id);
      ...
      <div className="absolute top-4 left-4 right-4 z-[1000] flex justify-between items-start pointer-events-none">
        <div className="bg-surface/90 backdrop-blur-md rounded-lg shadow-lg p-3 border border-outline-variant/50 pointer-events-auto">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="font-bold text-on-surface">Bus {assignment.bus?.bus_number}</h3>
          </div>
          <p className="text-body-sm text-secondary">Route: {assignment.route?.route_name}</p>
      ...
      <MapContainer center={position} zoom={15} maxBounds={SURAT_BOUNDS} maxBoundsViscosity={1.0} className="flex-1 w-full z-0" zoomControl={false}>
        ...
        {currentLocation && (
          <Marker position={position} icon={busIcon}>
            <Popup>
              <strong>Bus {assignment.bus?.bus_number}</strong><br/>
